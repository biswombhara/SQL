...
SELECT Continent, Country_Territory, _2022_Population,
       RANK() OVER (PARTITION BY Continent ORDER BY _2022_Population DESC) AS Continent_Rank,
       DENSE_RANK() OVER (PARTITION BY Continent ORDER BY _2022_Population DESC) AS Continent_DenseRank,
       ROW_NUMBER() OVER (PARTITION BY Continent ORDER BY _2022_Population DESC) AS Continent_Row_Rank
FROM world_population;
...
SELECT Continent, SUM(_2022_Population) AS Total_2022_Population
FROM world_population
GROUP BY Continent
